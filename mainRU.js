...
function getCountryData(countryName, countryEng) {
    ...
    .then(result => {
        ...
        if (result.cases == null) {document.getElementById('cases').innerHTML = 'Информация недоступна'} 
        else {document.getElementById('cases').innerHTML = result.cases.toLocaleString('en')}
        if (result.recovered == null) {document.getElementById('recovered').innerHTML = 'Информация недоступна'} 
        else {document.getElementById('recovered').innerHTML = result.recovered.toLocaleString('en')}
        if (result.active == null) {document.getElementById('active').innerHTML = 'Информация недоступна'} 
        else {document.getElementById('active').innerHTML = result.active.toLocaleString('en')}
        if (result.critical == null) {document.getElementById('critical').innerHTML = 'Информация недоступна'} 
        else {document.getElementById('critical').innerHTML = result.critical.toLocaleString('en')}
        if (result.deaths == null) {document.getElementById('deaths').innerHTML = 'Информация недоступна'} 
        else {document.getElementById('deaths').innerHTML = result.deaths.toLocaleString('en')}

        //Daily data
        if (result.todayCases == null) {document.getElementById('new-cases').innerHTML = 'Информация недоступна'} 
        else {document.getElementById('new-cases').innerHTML = '+' + result.todayCases.toLocaleString('en')}
        if (result.todayDeaths == null) {document.getElementById('new-deaths').innerHTML = 'Информация недоступна'} 
        ...
        if(result.recovered == null || result.active == null || result.deaths == null){
            document.getElementById('no-info').innerHTML = 'Информация недоступна'
    ...
    .catch(error => alert('Ошибка: Страна не найдена'), hamburger.classList.remove('open'))
};
        ...
        function drawChart() {
            if(result.recovered == null || result.active == null || result.deaths == null){
                var data = google.visualization.arrayToDataTable([
                    ['COVID-19 летальность', 'Процент летальности'],
                    ['Выздоравело',     0],
                    ['Заражено',     0],
                    ['Умерло',  0]
                ]);
                ...
                var data = google.visualization.arrayToDataTable([
                    ['COVID-19 летальность', 'Процент летальности'],
                    ['Выздоравело',     result.recovered],
                    ['Заражено',     result.active],
                    ['Умерло',  result.deaths]
                ]);
            }
